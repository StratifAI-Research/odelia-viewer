...
export const SeriesSelectionStep: React.FC<SeriesSelectionStepProps> = ({
  ...
  return (
    <div className="relative flex-1 flex flex-col min-h-0 h-full">
      {/* Content area - scrollable with padding for fixed button */}
      <div className="flex-1 min-h-0 px-3 pt-4 pb-20 space-y-3 overflow-y-auto overflow-x-hidden">
        {renderContent()}
      </div>

      {/* Button footer - fixed at bottom, always visible */}
      <div className="absolute bottom-0 left-0 right-0 px-3 py-3 border-t border-secondary-light bg-black">
        <Button
          onClick={onNext}
